...
impl eframe::App for DelayVstApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // --- DPI-Aware Stable Viewport Scaling (Prevents oscillation) ---
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ...
                // Main Container locked to exact 530px x 440px dimensions
...
fn draw_top_panel(ui: &mut egui::Ui, rect: egui::Rect, _app: &DelayVstApp, time: f32) {
    ...
    // --- Dynamic Neon Glowing Wireframe Line utility ---
    ...
    // --- Large Left Cube Group with float breathing + expansion scaling ---
    ...
    // --- Small Right Cube Group with phase shifted breathing ---
...
fn draw_mid_bar(ui: &mut egui::Ui, rect: egui::Rect, app: &mut DelayVstApp) {
    // 1. Perform UI actions and layout modifications first
    ...
    // 2. Fetch immutable painter after allocations are finished
    let painter = ui.painter();
    ...
    // Subtle switch track inner bevel
    painter.line_segment(
...
fn draw_bottom_panel(ui: &mut egui::Ui, rect: egui::Rect, app: &mut DelayVstApp, ctx: &egui::Context) {
    ...
    // --- flex "Space-Between" mathematically aligned positions ---
    ...
    // Set cursor styles based on hovered states
    ...
    // Top metal catch-light reflection line
    painter.line_segment(
    ...
    // --- Elegant 4-pointed Concave Sparkle Stars cluster ---
    draw_filter_button(painter, sparkle_rect, "SPARKLE", app.sparkle_active, |p, center| {
        ...
        // Precise math generation of 4-point flares using quadratic Beziers (Curves inward smoothly to (0,0))
        let draw_quadratic_bezier_sparkle = |p_paint: &egui::Painter, c: egui::Pos2, radius: f32| {
            ...
            for i in 0..4 {
                ...
                for step in 0..num_samples {
                    ...
                    let b = p0 * (1.0 - t).powi(2) + p2 * t.powi(2); // quadratic bezier evaluated with control point at (0,0)
                    points.push(c + b);
                }
            }
        ...
        // Offset configuration matching coordinates of the original HTML SVG layout
        let stars = [
    ...
    let draw_vu_meter = |p: &egui::Painter, meter_cx: f32, level: f32, label: &str| {
        ...
        // Inset shadow detail in the LED recess tracks
...
// Inline constant lengths mapping helpers
fn tempo_values_len() -> usize {
    5 // Last index of ["1/32", "1/16", "1/8", "1/4", "1/2", "1/1"] is 5
}

fn draw_knob(painter: &egui::Painter, center: egui::Pos2, radius: f32, val: f32) {
    // --- Outer Dashed-Dot Track Ring (border: 1px dashed rgba(160, 174, 192, 0.45)) ---
    let num_dots = 16;
    for i in 0..num_dots {
    ...
    // --- Outer Drop Shadow: mimicking box-shadow 0 3px 6px rgba(0,0,0,0.12) perfectly ---
    let shadow_offset = egui::vec2(0.0, 3.0);
    painter.circle_filled(center + shadow_offset, radius + 3.0, egui::Color32::from_rgba_unmultiplied(0, 0, 0, 10)); // 4% soft blur radius expansion
    painter.circle_filled(center + shadow_offset * 0.7, radius + 1.5, egui::Color32::from_rgba_unmultiplied(0, 0, 0, 18)); // 7% mid blur
    painter.circle_filled(center + shadow_offset * 0.4, radius, egui::Color32::from_rgba_unmultiplied(0, 0, 0, 24)); // 9% sharp occlusion shadow

    // --- Base Cylinder Outer Border (border: 1.2px solid #b8c8d5) ---
    painter.circle_filled(center, radius, egui::Color32::from_rgb(184, 200, 213)); // #b8c8d5

    // --- Inset Bottom Shadow (inset 0 -1.5px 2px rgba(0,0,0,0.15)) ---
    // Draw a darker circle shifted upwards inside the knob base
    painter.circle_filled(center - egui::vec2(0.0, 1.0), radius - 1.2, egui::Color32::from_rgb(160, 175, 190));

    // --- Inset Top Highlight (inset 0 1.5px 2px rgba(255,255,255,0.85)) ---
    // Draw a white circle shifted downwards inside the knob base
    painter.circle_filled(center + egui::vec2(0.0, 1.0), radius - 1.2, egui::Color32::WHITE.linear_multiply(0.85));

    // --- Knob Metallic Face (radial-gradient(circle at 35% 35%, #f1f6fa 0%, #cbdbe7 100%)) ---
    painter.circle_filled(center, radius - 2.5, egui::Color32::from_rgb(203, 219, 231)); // #cbdbe7
    
    // Highlight offset upwards-left to match light source direction
    let shine_offset = egui::vec2(-radius * 0.15, -radius * 0.15);
    painter.circle_filled(center + shine_offset, (radius - 2.5) * 0.85, egui::Color32::from_rgb(241, 246, 250)); // #f1f6fa
    ...
    // Pointer dot offset perfectly near edge (.knob-pointer top: 6px equivalent)
    let pointer_dist = radius - 5.5;
    let pointer_pos = center + egui::vec2(angle_rad.cos(), angle_rad.sin()) * pointer_dist;
    painter.circle_filled(pointer_pos, 2.0, egui::Color32::from_rgb(74, 85, 104)); // #4a5568 pointer
}
